...
cmd:option('--earlystop', 20, 'maximum number of epochs to wait to find a better local minima for early-stopping')
...
cmd:option('--savepath', paths.concat(dl.SAVE_PATH, 'anraGRUlm'), 'path to directory where experiment log (includes model) will be saved')
cmd:option('--id', '530', 'id string of this experiment (used to name output file) (defaults to a unique id)')
...
for i,hiddensize in ipairs(opt.hiddensize) do 
	...
	if opt.gru then -- Gated Recurrent Units
		rnn = nn.anraGRU(inputsize, hiddensize, nil, opt.dropout/2)
