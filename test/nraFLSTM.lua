local nraFLSTM, parent = torch.class("nn.nraFLSTM", "nn.LSTM")
...
function nraFLSTM:__init(inputSize, outputSize, rho, eps, momentum, affine)
...
function nraFLSTM:buildModel()
	...
	transfer:add(nn.Sigmoid()):add(getresmodel(nn.Tanh(),0.2)):add(nn.Sigmoid()):add(nn.Sigmoid())
	gates:add(transfer)
	...
	cellAct:add(getresmodel(nn.Tanh(),0.2))
	local concat = nn.ConcatTable()
...
function nraFLSTM:nngraphModel()
	...
	local in_transform = getresmodel(nn.Tanh(),0.2)(n2)
	...
	if self.bn then
		...
		next_h = nn.CMulTable()({out_gate, getresmodel(nn.Tanh(),0.2)(bn_c(next_c):annotate {name = 'bn_c'}) })
	else
		-- gated cells form the output
		next_h = nn.CMulTable()({out_gate, getresmodel(nn.Tanh(),0.2)(next_c)})
	end
...
function nraFLSTM:buildGate()
	error"Not Implemented"
end

function nraFLSTM:buildInputGate()
	error"Not Implemented"
end

function nraFLSTM:buildForgetGate()
	error"Not Implemented"
end

function nraFLSTM:buildHidden()
	error"Not Implemented"
end

function nraFLSTM:buildCell()
	error"Not Implemented"
end	
	
function nraFLSTM:buildOutputGate()
	error"Not Implemented"
end
