...
local nraLSTM, parent = torch.class('nn.nraLSTM', 'nn.AbstractRecurrent')
...
function nraLSTM:__init(inputSize, outputSize, rho, cell2gate)
...
function nraLSTM:buildHidden()
	...
	hidden:add(getresmodel(nn.Tanh(),0.2))
	self.hiddenLayer = hidden
	return hidden
end

function nraLSTM:buildCell()
	-- build
...
function nraLSTM:buildOutputGate()
...
------------------------- forward backward -----------------------------
function nraLSTM:updateOutput(input)
...
function nraLSTM:_updateGradInput(input, gradOutput)
...
function nraLSTM:_accGradParameters(input, gradOutput, scale)
