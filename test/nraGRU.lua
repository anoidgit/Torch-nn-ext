...
local nraGRU, parent = torch.class('nn.nraGRU', 'nn.AbstractRecurrent')
...
function nraGRU:__init(inputSize, outputSize, rho, p, mono)
...
function nraGRU:buildModel()
   ...
   hidden:add(concat):add(nn.CAddTable()):add(getresmodel(nn.Tanh(),0.2))
...
------------------------- forward backward -----------------------------
function nraGRU:updateOutput(input)
   local prevOutput
   if self.step == 1 then
...
function nraGRU:_updateGradInput(input, gradOutput)
...
function nraGRU:_accGradParameters(input, gradOutput, scale)
