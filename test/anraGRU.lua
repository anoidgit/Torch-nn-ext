...
local anraGRU, parent = torch.class('nn.anraGRU', 'nn.AbstractRecurrent')
...
function anraGRU:__init(inputSize, outputSize, rho, p, mono)
...
function anraGRU:buildModel()
	...
	hidden:add(concat):add(nn.CAddTable()):add(getresmodel(nn.Tanh(),nn.Mul()))
...
------------------------- forward backward -----------------------------
function anraGRU:updateOutput(input)
	local prevOutput
	if self.step == 1 then
...
function anraGRU:_updateGradInput(input, gradOutput)
...
function anraGRU:_accGradParameters(input, gradOutput, scale)
